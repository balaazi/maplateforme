...
class LieuDisponibiliteService
{
    ...
    /**
     * Vérifie si un lieu est disponible à un moment donné
     */
    public function estDisponible(Lieu $lieu, \DateTimeInterface $debut, \DateTimeInterface $fin): bool
    {
        // Vérifier que le lieu n'est pas désactivé
        if (!$lieu->isDisponible()) {
            return false;
        }

        // Vérifier que la période demandée est dans les heures d'ouverture du lieu
        if (!$this->estDansHeuresOuverture($lieu, $debut, $fin)) {
            return false;
        }

        // Vérifier les horaires spécifiques par jour de la semaine
        if (!$this->estDansHorairesParJour($lieu, $debut, $fin)) {
            return false;
        }
    ...
    /**
     * Vérifie si un lieu est actuellement occupé
     */
    public function estOccupee(Lieu $lieu): bool
    {
        $maintenant = new \DateTime();
        
        $reservationActuelle = $this->reservationRepository->findReservationActuelle($lieu, $maintenant);
        
        return $reservationActuelle !== null;
    }

    /**
     * Récupère le statut actuel d'un lieu
     */
    public function getStatutActuel(Lieu $lieu): array
    {
        if (!$lieu->isDisponible()) {
            return [
                'statut' => 'desactivee',
        ...
        $prochaineReservation = $this->reservationRepository->findProchaineReservation($lieu, $maintenant);
    ...
    /**
     * Récupère le statut d'un lieu pour une date spécifique
     */
    public function getStatutActuelPourDate(Lieu $lieu, \DateTimeInterface $date): array
    {
        if (!$lieu->isDisponible()) {
            return [
                'statut' => 'desactivee',
        ...
        $reservationsPourDate = $this->reservationRepository->findReservationsPourJour($lieu, $date);
        ...
        if (!empty($reservationsPourDate)) {
            ...
            if ($date->format('Y-m-d') === $maintenant->format('Y-m-d')) {
                ...
                // Pour une date future, le lieu est réservé
                return [
                    'statut' => 'reservee',
                    'libelle' => 'Réservé',
                    'couleur' => 'warning',
    ...
    /**
     * Récupère les créneaux libres d'un lieu pour une journée donnée
     */
    public function getCreneauxLibres(Lieu $lieu, \DateTimeInterface $date): array
    {
        ...
        $reservations = $this->reservationRepository->findReservationsPourJour($lieu, $date);
    ...
    /**
     * Vérifie si la période est dans les heures d'ouverture du lieu
     */
    private function estDansHeuresOuverture(Lieu $lieu, \DateTimeInterface $debut, \DateTimeInterface $fin): bool
    {
        // Vérifier avec les heures de début et fin de réservation du lieu
        $heureOuverture = $lieu->getDebutReservation();
        $heureFermeture = $lieu->getFinReservation();
        ...
        if ($fermetureHeure < $ouvertureHeure) {
            // Cas de passage minuit : lieu ouvert de 22:00 à 02:00
        ...
        // Cas normal : lieu ouvert de 08:00 à 18:00
     ...
     */
    private function estDansHorairesParJour(Lieu $lieu, \DateTimeInterface $debut, \DateTimeInterface $fin): bool
    {
        $horairesParJour = $lieu->getHorairesParJour();
    ...
    /**
     * Trouve un lieu disponible avec la capacité demandée
     */
    public function trouverLieuDisponible(int $capaciteMinimum, \DateTimeInterface $debut, \DateTimeInterface $fin): ?Lieu
    {
        $lieux = $this->entityManager->getRepository(Lieu::class)
            ->findBy(['disponible' => true]);

        foreach ($lieux as $lieu) {
            if ($lieu->getCapacite() >= $capaciteMinimum && 
                $this->estDisponible($lieu, $debut, $fin)) {
                return $lieu;
            }
        }
